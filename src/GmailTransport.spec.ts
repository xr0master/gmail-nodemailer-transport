...
test('test the attachments path', (done: Function) => {
  ...
  createTransport(new GmailTransport(<Options>{
    ...
    subject: 'Gmail Transport Attachment Test',
    html: '<!DOCTYPE html><html><body><img src="cid:cat" alt="cat"></body></html>',
    attachments: [<Attachment>{
      cid: 'cat',
      path: 'https://emailjs-dev-attachments.s3.us-west-2.amazonaws.com/user_LC2JWGNosRSeMY6HmQFUn/feedback_template/cat.jpg'
    }]
  }).then((info) => {
    expect(info).toBeDefined();
    done();
...
test('test the attachments base64', (done: Function) => {
  expect.assertions(1);
...
test('test the Gmail error', (done: Function) => {
  expect.assertions(1);
